...
//    }
    ...
    public Double getCurrentMonthRevenue() {
        Calendar cal = Calendar.getInstance();
        int month = cal.get(Calendar.MONTH) + 1; // Calendar: 0-based month
        int year  = cal.get(Calendar.YEAR);

        Double revenue = ordersRepository.getRevenueOfMonth(month, year);
    ...
    public Long getCurrentMonthOrders() {
        ...
        Long count = ordersRepository.countOrdersOfMonth(month, year);
        return count != null ? count : 0L;
    }

    public int getCurrentMonthGrowthRate() {
        Calendar cal = Calendar.getInstance();
        ...
        double current = Optional.ofNullable(
                ordersRepository.getRevenueOfMonth(currentMonth, currentYear)
        ).orElse(0.0);

        double previous = Optional.ofNullable(
                ordersRepository.getRevenueOfMonth(prevMonth, prevYear)
        ).orElse(0.0);

        if (previous == 0) {
            return current > 0 ? 100 : 0;  // nếu tháng trước 0 mà tháng này > 0 → xem như +100%
        }
    ...
    public Double getAverageOrderValueThisMonth() {
        double revenue = getCurrentMonthRevenue();
        long orders = getCurrentMonthOrders();

        if (orders == 0) return 0.0;
        return revenue / orders;
    }

    public List<Books> getTop8BestSellingBooks() {
        return booksRepository.findTopBestSelling(PageRequest.of(0, 8));
    }
